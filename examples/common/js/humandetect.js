	...
	var detector = (function() {
		...
		function detector(width, height, scaleFactor, classifier) {
			...
			for (var i = 0; i < this.numScales; ++i) {
				...
				this.compiledClassifiers[i] = humandetect.compileClassifier(classifier, scaledWidth);
				scale *= scaleFactor;
			}
		}
		...
		detector.prototype.detect = function(image, group, stepSize, roi, canny) {
			...
			this.gray = humandetect.convertRgbaToGrayscale(imageData, this.gray);
			...
			for (var i = 0; i < this.numScales; ++i) {
				...
				if (scale === 1) {
					...
					this.scaledGray = humandetect.rescaleImage(this.gray, width, height, scale, this.scaledGray);
				}
				
				if (canny) {
					this.canny = humandetect.computeCanny(this.scaledGray, scaledWidth, scaledHeight, this.canny);
					this.cannySat = humandetect.computeSat(this.canny, scaledWidth, scaledHeight, this.cannySat);
				}
				
				this.sat = humandetect.computeSat(this.scaledGray, scaledWidth, scaledHeight, this.sat);
				this.ssat = humandetect.computeSquaredSat(this.scaledGray, scaledWidth, scaledHeight, this.ssat);
				if (this.tilted) this.rsat = humandetect.computeRsat(this.scaledGray, scaledWidth, scaledHeight, this.rsat);

				var newRects = humandetect.detect(this.sat, this.rsat, this.ssat, this.cannySat, scaledWidth, scaledHeight, stepSize, this.compiledClassifiers[i]);
			...
			return (group ? humandetect.groupRectangles(rects, group) : rects).sort(function(r1, r2) {return r2[4] - r1[4];});
		};
